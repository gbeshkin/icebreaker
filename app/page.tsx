...
export default function Home() {
  ...
  async function loadEntries() { try { const response = await fetch("/api/entries", { cache: "no-store" }); const data = await response.json() as { entries?: Entry[]; error?: string }; if (!response.ok) throw new Error(data.error || "Could not load the gallery."); setEntries(data.entries ?? []); setError(""); } catch (err) { setError(err instanceof Error ? err.message : "Could not load the gallery."); } finally { setLoadingFeed(false); } }
  ...
  async function shareInterpretation(event: FormEvent) { event.preventDefault(); if (selectedCard === null || !name.trim() || !answer.trim()) return; setSaving(true); setError(""); try { const response = await fetch("/api/entries", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: name.trim(), cardId: selectedCard, answer: answer.trim() }) }); const data = await response.json() as { entry?: Entry; error?: string }; const entry = data.entry; if (!response.ok || !entry) throw new Error(data.error || "Your interpretation could not be shared."); setEntries((previous) => [entry, ...previous]); setAnswer(""); } catch (err) { setError(err instanceof Error ? err.message : "Your interpretation could not be shared."); } finally { setSaving(false); } }
